...
interface MeetingData {
  ...
  type?: string;
  objective?: string;
  teamPhotos?: string | string[];
  createdAt?: string;
  attendees: Attendee[];
  organizer?: { name: string; position?: string; company?: string };
  company?: { name: string }; // <-- AQUI ESTÁ A CORREÇÃO QUE FALTAVA!
}
...
export function generateDdsPdf(meeting: MeetingData) {
  ...
  doc.setFont('helvetica', 'bold');
  doc.text('Organizador:', 14, currentY + 12);
  doc.setFont('helvetica', 'normal');
  doc.text(meeting.organizer?.name || 'Não identificado', 40, currentY + 12);

  doc.setFont('helvetica', 'bold');
  doc.text('Data e Horário:', 14, currentY + 18);
  doc.setFont('helvetica', 'normal');
  doc.text(new Date(meeting.createdAt || Date.now()).toLocaleString('pt-BR'), 40, currentY + 18);

  doc.setFont('helvetica', 'bold');
  doc.text('Total de Participantes:', 14, currentY + 24);
  doc.setFont('helvetica', 'normal');
  doc.text(`${meeting.attendees.length} colaborador(es) auditado(s)`, 52, currentY + 24);
  ...
  const docInternal = doc.internal as unknown as { getNumberOfPages: () => number, pageSize: { height: number } };
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.setTextColor(100, 116, 139);
    doc.text(
      `DDS ON MASTER • Consulta Global (amtst.vercel.app) | Página ${i} de ${pageCount}`,
      105,
      docInternal.pageSize.height - 8,
      { align: 'center' }
    );
  }

  const cleanTopic = (meeting.topic || 'DDS').replace(/[^a-zA-Z0-9]/g, '_');
  doc.save(`DDS_ON_MASTER_Ata_${cleanTopic}_${new Date().toISOString().slice(0, 10)}.pdf`);
}
...
export function generateConsolidatedDdsPdf(report: ConsolidatedReportData) {
  ...
  doc.setFont('helvetica', 'bold');
  doc.text('Emitido por (Super Admin):', 14, 38);
  doc.setFont('helvetica', 'normal');
  doc.text(`${report.organizerName}`, 60, 38);

  const totalAttendees = report.meetings.reduce((acc, m) => acc + (m.attendees?.length || 0), 0);

  doc.setFont('helvetica', 'bold');
  doc.text('Total de Reuniões Globais:', 130, 38);
  doc.setFont('helvetica', 'normal');
  doc.text(`${report.meetings.length} DDSs realizados`, 175, 38);

  doc.setFont('helvetica', 'bold');
  doc.text('Total de Presenças:', 130, 44);
  doc.setFont('helvetica', 'normal');
  doc.text(`${totalAttendees} presenças auditadas`, 165, 44);
  ...
  const tableData = report.meetings.map(m => [
    ...
    m.topic,
    m.company?.name || m.farm || 'Geral',
    m.organizer?.name || 'Técnico',
  ...
  autoTable(doc, {
    startY: 55,
    head: [['Data', 'Tema do DDS', 'Empresa / Local', 'Organizador', 'Tipo', 'Qtd']],
    body: tableData,
    theme: 'striped',
    headStyles: {
      fillColor: [21, 128, 61],
  ...
  doc.save(`DDS_ON_MASTER_Global_${new Date().toISOString().slice(0, 10)}.pdf`);
}